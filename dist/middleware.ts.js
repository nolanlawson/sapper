...
function middleware(_a) {
    var routes = _a.routes, store = _a.store;
    var output = __chunk2_js.locations.dest();
...
function get_route_handler(chunks, routes, store_getter) {
    var template = __chunk2_js.dev()
        ? function () { return fs.readFileSync(__chunk2_js.locations.app() + "/template.html", 'utf-8'); }
        : (function (str) { return function () { return str; }; })(fs.readFileSync(__chunk2_js.locations.dest() + "/template.html", 'utf-8'));
    function handle_route(route, req, res) {
        ...
        if (route.type === 'page') {
            ...
            Promise.resolve(mod.preload ? mod.preload.call({
                ...
                var has_service_worker = fs.existsSync(path.join(__chunk2_js.locations.dest(), 'service-worker.js'));

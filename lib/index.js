...
function connect_dev() {
	...
	const middleware = compose_handlers([
		...
		get_asset_handler({
			...
			type:   pathname => (pathname.endsWith('.js')
				? 'application/javascript' : pathname.endsWith('.css')
					? 'text/css' : pathname.endsWith('.map') ? 'application/json' : 'application/octet-stream'),
...
function get_asset_handler(opts) {
	return (req, res, next) => {
		...
		const chunk = opts.fn(req.pathname)

		res.set({
			'Content-Type': opts.type(req.pathname),
...
function generate_styles_tags(client, css) {
	return (
		// just inline main.css because it's small
		client.main_css_file_content
			? `<style>${client.main_css_file_content}</style>\n`
			: ''
	) +
	(
		(css && css.code)
			...
			for (const route of route_manager.routes) {
				if (route.test(url)) {
					...
					if (route.type === 'page') {
						...
						res.set('Link',
							[
								`<${client.main_file}>;rel="preload";as="script"`,
								`<${client.routes[route.id].js}>;rel="preload";as="script"`,
								client.routes[route.id].css && `<${client.routes[route.id].css}>;rel="preload";as="style"`
							].filter(Boolean).join(',')
						);
						...
						if (mod.preload) {
							...
							await templates.stream(res, 200, {
								...
								styles: promise.then(({ css }) => generate_styles_tags(client, css))
							});
						} else {
							...
							const page = templates.render(200, {
								...
								styles: generate_styles_tags(client, css)
							});

							const buff = Buffer.from(page, 'utf-8');
